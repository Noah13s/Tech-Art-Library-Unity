...
public class VolumetricClouds : VolumeComponent, IPostProcessComponent
{
    ...
    public CloudPresets cloudPreset
    {
        ...
        set { m_CloudPreset.overrideState = true; m_CloudPreset.value = value; ApplyCurrentCloudPreset(); }
    }
    ...
    [Tooltip("Controls the average amount of the planet covered by the large-scale weather pattern.")]
    public ClampedFloatParameter planetaryCoverage = new(0.48f, 0.0f, 1.0f);

    [Tooltip("Controls the definition of boundaries between clear and cloudy weather systems.")]
    public ClampedFloatParameter planetaryCoverageContrast = new(0.78f, 0.0f, 1.0f);

    [Tooltip("Controls how strongly planet-wide coverage breaks up the local cloud pattern at close range.")]
    public ClampedFloatParameter planetaryCoverageInfluence = new(0.45f, 0.0f, 1.0f);
    ...
    [Tooltip("Physical altitude in metres where the planetary pattern fully replaces fine cloud detail.")]
    public MinFloatParameter planetaryDetailFadeEnd = new(240000.0f, 1.0f);

    [Header("Orbital Proxy")]
    [Tooltip("Optical density of the analytic cloud shell used once local volumetric detail has faded out.")]
    public ClampedFloatParameter orbitalProxyOpacity = new(0.75f, 0.25f, 5.0f);

    [Tooltip("Color tint applied to the satellite-scale analytic cloud shell.")]
    public ColorParameter orbitalProxyTint = new(Color.white);

    [Tooltip("Minimum illumination retained by the analytic shell on the night side.")]
    public ClampedFloatParameter orbitalProxyAmbient = new(0.12f, 0.0f, 1.0f);
