...
#region Variables
    ...
    Color baseColor = new Color(255, 255, 255, 0.5f);
    ...
    private void UpdateDebugVisuals()
    {
        ...
        foreach (var wheel in wheels)
        {
            ...
            if (wheel.wheelData.isSteering)
            {
                ...
                Handles.DrawLine(new Vector3(wheel.transform.position.x, averageWheelsHeight, wheel.transform.position.z), new Vector3(wheel.transform.position.x, averageWheelsHeight, wheel.transform.position.z) + (Quaternion.Euler(0, currentSteeringAngle, 0) * transform.forward * 1f));
            }
            else
            {
            }
        ...
        Handles.DrawLine(p3, p2);//   Draws a green line on the wedge to indicate the direction
        #endregion
...
public class CustomEditorVehicle_Player : Editor
{
    ...
    void ValidateVehicleSetup()
    {
        ...
        if (playerScript.GetComponent<Rigidbody>() == null)
        {
            EditorGUILayout.HelpBox("No rigidbody found at root of vehicle.", MessageType.Error);
        }
